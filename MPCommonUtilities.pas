...
procedure ConvertBitmapEx(
  const Image32: TBitmap;
  OutImage: TBitmap;
...
function UsesAlphaChannel(const Image32: TBitmap): Boolean;
...
begin
  Exit;
  // This creates a white background for folders !!!

  // Fast exit — no alpha, no work
  if (Image32 = nil) or
     (Image32.PixelFormat <> pf32bit) or
     not UsesAlphaChannel(Image32) then
  begin
    OutImage.Assign(Image32);
    Exit;
  end;
  ...
  // Prepare output
  OutImage.SetSize(Image32.Width, Image32.Height);
  OutImage.PixelFormat := pf32bit;

  // Extract background once (not per pixel)
  BgR := GetRValue(ColorToRGB(BackGndColor));
  BgG := GetGValue(ColorToRGB(BackGndColor));
  BgB := GetBValue(ColorToRGB(BackGndColor));

  for Y := 0 to Image32.Height - 1 do
  begin
    Src := PRGBA(Image32.ScanLine[Y]);
    Dst := PRGBA(OutImage.ScanLine[Y]);

    for X := 0 to Image32.Width - 1 do
    begin
      A := Src^.A; // 0..255

      // Proper alpha blend (256-based, no overflow)
      Dst^.R := (Src^.R * A + BgR * (256 - A)) shr 8;
      Dst^.G := (Src^.G * A + BgG * (256 - A)) shr 8;
      Dst^.B := (Src^.B * A + BgB * (256 - A)) shr 8;
      Dst^.A := 255; // flattened, opaque
...
begin
  ...
  // Hard guards — fail fast, no assumptions
  if (Image32 = nil) or
     (Image32.PixelFormat <> pf32bit) or
     (Image32.Width = 0) or
     (Image32.Height = 0) then
    Exit;

  for Y := 0 to Image32.Height - 1 do
  begin
    Row := PRGBA(Image32.ScanLine[Y]);

    // Tight inner loop, predictable memory access
    for X := 0 to Image32.Width - 1 do
    begin
      if Row^.A <> 0 then
        Exit(True);
