...
//
// 21 Dec 2025 - by vhl - improvements for thread safety and fastmm5 friendly
...
type
  ...
  {$ENDIF}
    ...
    procedure WMXButtonDown(var Msg: TMessage); message WM_XBUTTONDOWN;
  protected
...
begin
  ...
  if Assigned(Column) and TExplorerColumn(Column).IsCustom then
    ...
    begin
      ...
      begin
        Result := NS2.ComparePIDL(NS1.RelativePIDL, False, ColumnIndex);                 // no optimized code

        // Secondary level of sorting is on the Name
        if (Result = 0) and (ColumnIndex > 0) then
           Result := NS2.ComparePIDL(NS1.RelativePIDL, False, 0);
//        // Optimization: Use fast CompareText for Name column (0) instead of Shell CompareIDs !revert to comment code if strict shell ordering is required
//        if ColumnIndex = 0 then
//          Result := CompareText(NS1.NameInFolder, NS2.NameInFolder)
//        else
//          Result := NS2.ComparePIDL(NS1.RelativePIDL, False, ColumnIndex);
//        //Secondary level of sorting is on the Name
//        if (Result = 0)  and (ColumnIndex > 0) then
//          Result := CompareText(NS1.NameInFolder, NS2.NameInFolder);     //rever until here
    ...
    begin
      case GroupingColumn of
        0: begin  // Name Grouping
             ...
             begin
               if Groups[i].Caption[1] = System.UpCase( NS.NameInFolder[1]) then
...
//                    Result := TExplorerGroup(Groups[i])
                 if SameText(Groups[i].Caption, GroupingFileSizeArray[Length(GroupingFileSizeArray) - 1].Caption) then
                   Result := TExplorerGroup(Groups[i])
               end else
               begin
...
//                     if WideStrIComp(PWideChar(Groups[i].Caption), PWideChar(WS)) = 0 then
//                       Result := TExplorerGroup(Groups[i]);
                     if SameText(Groups[i].Caption, GroupingFileSizeArray[iArray].Caption) then
                       Result := TExplorerGroup(Groups[i]);
                     Done := True
                   end;
                   Inc(iArray);
...
//               if WideStrIComp(PWideChar(Groups[i].Caption), PWideChar(WS)) = 0 then
//                 Result := TExplorerGroup(Groups[i])
              if SameText(Groups[i].Caption, GroupingFileSizeArray[Length(GroupingFileSizeArray) - 2].Caption) then
                Result := TExplorerGroup(Groups[i]);
             end
           end;
...
//                   Result := TExplorerGroup(Groups[i]);
                 if SameText(Groups[i].Caption, GroupingModifiedArray[iArray].Caption) then
                    Result := TExplorerGroup(Groups[i]);
                 Done := True
               end;
               Inc(iArray)
...
//        Result := TExplorerGroup(Groups[i])
      if SameText(Groups[i].Caption, NS.DetailsOf(GroupingColumn)) then
        Result := TExplorerGroup(Groups[i])
    end;
    Inc(i)
  end;
...
begin
  var LNS := TExplorerItem(Item).Namespace; // caching to improve bottleneck
  if CompressedFile.Hilight and (Position = 0) then
    if LNS.Compressed and not Item.Selected then
    begin
      var LFont := CompressedFile.Font;
      ACanvas.Font.Color := LFont.Color;//.Assign(CompressedFile.Font); // Assign is very expensive in VCL, copies all font properties and triggers internal on change events
      ACanvas.Font.Style := LFont.Style;
    end;
  if EncryptedFile.Hilight and (Position = 0) then
    if LNS.Encrypted and not Item.Selected then
    begin
      var LFont := EncryptedFile.Font;
      ACanvas.Font.Color := LFont.Color;//.Assign(EncryptedFile.Font);
      ACanvas.Font.Style := LFont.Style;
    end;
...
//  if ExtensionColorCode and Assigned(TExplorerItem(Item).Namespace) and TExplorerItem(Item).Namespace.FileSystem and ((ExtensionColorCodeSelected and Item.Selected) or not Item.Selected) then
  if ExtensionColorCode and Assigned(LNS) and LNS.FileSystem and ((ExtensionColorCodeSelected and Item.Selected) or not Item.Selected) then
  begin
    ColorCode := ExtensionColorCodeList.FindColorCode(LNS);
...
//          ACanvas.Handle := 0;
//          ACanvas.Free;
//        end // let's use self canvas to avoid TCanvas.Create overhead
        if HandleAllocated then
        begin
          Canvas.Lock;
          try
            // Save current font state if necessary, LoadTextFont usually sets it
            Item.View.LoadTextFont(Item, Canvas, True);
            TextSize := TextExtentW(Item.Caption, Canvas);
          finally
            Canvas.Unlock;
          end;
        end else
...
//workaround to force an item selection to do the context menu, previous original code above commented
procedure TCustomVirtualExplorerEasyListview.DoItemContextMenu(
  HitInfo: TEasyHitInfoItem; WindowPoint: TPoint; var Menu: TPopupMenu;
  var Handled: Boolean);
begin
  ...
  // FIX: Auto-select item on right-click if not already selected
  if not Handled and Assigned(HitInfo.Item) and (eloShellContextMenus in Options) then
  begin
    if not HitInfo.Item.Selected then
    begin
...
//        Selection.EndUpdate;
    ...
    if HitInfo.HitInfo * [ehtOnText, ehtOnIcon] <> [] then
    begin
      FContextMenuItem := HitInfo.Item;
      try
        TExplorerItem(HitInfo.Item).Namespace.ShowContextMenuMulti(Self, ContextMenuCmdCallback,
...
begin
  ...
  begin
    ...
    begin
      ...
      // lower prority for details to prevent scrolling stutter
      TileRequest.Priority := 5; //10;
  ...
  begin
    ...
    begin
      ...
      if TempNS.Folder and (eloBrowseExecuteFolder in Options) then
        ...
        begin
          if eloBrowseExecuteZipFolder in Options then
          // these comments out blocks to open the folder in the file explorer of windows
//            DefaultExecution := (eloBrowseExecuteZipFolder in Options) and not
             BrowseToNextLevel; // This works because the click has selected the node
//          else
...
//          DefaultExecution := not BrowseToNextLevel; // This works because the click has selected the node
      ...
      begin
       // Notify Windows that this file is being accessed || by vhl
        if not TempNS.Folder then
          SHAddToRecentDocs(SHARD_PIDL, TempNS.AbsolutePIDL);
        ...
        begin
          ...
          begin
            ...
            if eloUseColumnOnByDefaultFlag in Options then
              Column.Visible := csOnByDefault in LNS.DetailsGetDefaultColumnState(Column.Index);
            {$ELSE}
...
//        Selection.FocusedColumn.SortDirection := DefaultSortDir
      var LCol := Header.Columns[DefaultSortColumn]; // resolving fast, small improvement but may improve some bottleneck
      Selection.FocusedColumn := LCol;
      if DefaultSortDir <> esdNone then
        LCol.SortDirection := DefaultSortDir;
    end
    else
...
procedure TCustomVirtualExplorerEasyListview.WMXButtonDown(var Msg: TMessage);
begin
  // HIWORD(wParam) = 1 for XBUTTON1 (Back), 2 for XBUTTON2 (Forward)
...
//procedure FlipBitmapVertical(Bitmap: TBitmap);
//var
//  TopRow, BottomRow: Pointer;
//  RowSize: Integer;
//  Y, H: Integer;
//  TempRow: Pointer;
//begin
...
//
//  // Shell thumbnails are usually 32-bit (Alpha).
//  // We enforce this to ensure pixel math is consistent (4 bytes per pixel).
//  Bitmap.PixelFormat := pf32bit;
//
//  H := Bitmap.Height;
//  RowSize := Bitmap.Width * 4; // 32-bit = 4 bytes per pixel
//
//  GetMem(TempRow, RowSize);
//  try
//    // Swap rows from top to bottom
//    for Y := 0 to (H div 2) - 1 do
//    begin
//      TopRow := Bitmap.ScanLine[Y];
//      BottomRow := Bitmap.ScanLine[H - 1 - Y];
//
//      // Swap memory content
//      Move(TopRow^, TempRow^, RowSize);
//      Move(BottomRow^, TopRow^, RowSize);
//      Move(TempRow^, BottomRow^, RowSize);
//    end;
//  finally
//    FreeMem(TempRow);
//  end;
...
var
  Y, H, RowSize: Integer;
  TempRow: array of Byte;
  P1, P2: Pointer;
begin
  if (Bitmap = nil) or (Bitmap.Height < 2) then Exit;
  Bitmap.PixelFormat := pf32bit;
  H := Bitmap.Height;
  RowSize := Bitmap.Width shl 2; // Fast * 4
  SetLength(TempRow, RowSize);
  for Y := 0 to (H shr 1) - 1 do // Fast div 2
  begin
    P1 := Bitmap.ScanLine[Y];
    P2 := Bitmap.ScanLine[H - 1 - Y];
    Move(P1^, TempRow[0], RowSize);
    Move(P2^, P1^, RowSize);
    Move(TempRow[0], P2^, RowSize);
  end;
end;
  ...
  try
    ...
    try
      ...
      FInternalThumbInfo := SpCreateThumbInfoFromFile(NS, ThumbSize.X, ThumbSize.Y,
        ...
        // let's fix upside down bitmaps from shell extraction from Windows standard DIB
        if Assigned(FInternalThumbInfo) and UseShellExtraction
        then
        begin
          var tempBmp := TBitmap.Create;
          try
            if FInternalThumbInfo.ReadBitmap(TempBmp) then
            begin
              var DIB: TDIBSection;
              if (tempBmp.Handle <> 0) and (GetObject(TempBmp.Handle, SizeOf(DIB), @DIB) = SizeOf(DIB)) then
              begin
              // Windows standard: biHeight > 0 is bottom-up. 32-bit thumbnails from IExtractImage/IThumbnailProvider often arrive this way and need
              // flipping for correct AlphaBlend/Canvas drawing.
                if DIB.dsBmih.biHeight > 0 then
                begin
                  FlipBitmapVertical(TempBmp);
                    FInternalThumbInfo.WriteBitmap(TempBmp);
                end;
              end;
      ...
      try
        ...
        // Prevent exception from propagating to thread loop
        on E: Exception do
          {$IFDEF GX_DEBUG}SendDebug('ThumbThread Error: ' + E.Message); {$ENDIF}
      end
    end;
  finally
    NS.Free;
...
var
  ...
  AlbumCache: TDictionary<string, TThumbInfo>;
begin
  // Optimization: If albumis empty, exit immediately
  if Album.Count = 0 then Exit;

  // Optimizaiton: Crate a dictionary for O(1) lookupof thumbnails by filename
  // This replaces the O(n)linear search (Album.IndexOf) inside the loop
  // preventing UI freezes in foldres with many files
  AlbumCache := TDictionary<string, TThumbInfo>.Create(TIStringComparer.Ordinal);
  try
    // populate the cache map
    for I := 0 to Album.Count - 1 do
    begin
      if Album.Read(I, AlbumT) then
        AlbumCache.AddOrSetValue(AlbumT.Filename, AlbumT);
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          // Check if image is already handled by a thread
          ...
          begin
            ...
            begin
              // Fast O(1) lookup from our temporary cache
              if AlbumCache.TryGetValue(NS.NameForParsing, AlbumT) then
              begin
                // Verify the timestamp matches to ensure thumbnail is up to date
