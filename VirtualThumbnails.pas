...
21 Dec 2025 - by vhl - WIC (.webp>win10 1809+ .png) and other improvements for thread safety and fastmm5 friendly
==============================================================================}
...
begin
  ...
  // 1. Try FAST Exif Thumbnail first (Zero decode time)
    ...
    try
      ...
      if Assigned(J) then begin
        ...
        try
          P.Assign(J);
          HasExifThumb := True;
          // Extract dimensions and orientation from Exif tags
          I := Exif.IndexOfName('$A002');
          if I > -1 then ImageWidth := StrToIntDef(Exif.ValueFromIndex[I], 0);
          I := Exif.IndexOfName('$A003');
          if I > -1 then ImageHeight := StrToIntDef(Exif.ValueFromIndex[I], 0);

          if ExifOrientation then begin
            I := Exif.IndexOfName('$112'); // Orientation tag
            if I > -1 then begin
              S := Exif.ValueFromIndex[I];
              if S = '6' then Orientation := 90
              else if S = '3' then Orientation := 180
          ...
          OutBitmap.Canvas.Lock;
          try
            DestR := SpRectAspectRatio(P.Graphic.Width, P.Graphic.Height, ThumbW, ThumbH, talNone);
            SpInitBitmap(OutBitmap, DestR.Right, DestR.Bottom, BgColor);
            SpStretchDraw(P.Graphic, OutBitmap.Canvas, DestR, Subsampling);
            if (Orientation > 0) then SpPixelRotate(OutBitmap, Orientation);
            Result := True;
          finally
            OutBitmap.Canvas.Unlock;
          end;
  ...
  // 2. Try WIC (Windows Imaging Component) - Fastest for PNG/JPG/TIFF/GIF
  // WIC scales on decode, avoiding large memory allocations
  UseWIC := (Ext = '.jpg') or (Ext = '.jpeg') or (Ext = '.png') or
            (Ext = '.bmp') or (Ext = '.gif') or (Ext = '.tif') or (Ext = '.tiff') or
            (Ext = '.webp');

  if UseWIC then
  try
    Factory := CreateComObject(CLSID_WICImagingFactory) as IWICImagingFactory;

    // Create Decoder
    if Succeeded(Factory.CreateDecoderFromFilename(PWideChar(WideString(Filename)), PGUID(nil)^, GENERIC_READ, WICDecodeMetadataCacheOnDemand, Decoder)) then
    begin
      if Succeeded(Decoder.GetFrame(0, Frame)) then
      begin
        Frame.GetSize(W, H);
        ImageWidth := Integer(W);
        ImageHeight := Integer(H);

        // Calculate Target Size
        DestR := SpRectAspectRatio(ImageWidth, ImageHeight, ThumbW, ThumbH, talNone);

        // Create Scaler (High quality Fant/Lanczos scaling happens here)
        if Succeeded(Factory.CreateBitmapScaler(Scaler)) then
        begin
          if Succeeded(Scaler.Initialize(Frame, DestR.Right, DestR.Bottom, WICBitmapInterpolationModeFant)) then
          begin
            // Convert pixels to 32-bit PBGRA (Compatible with TBitmap)
            if Succeeded(Factory.CreateFormatConverter(Converter)) then
            begin
              if Succeeded(Converter.Initialize(Scaler, GUID_WICPixelFormat32bppPBGRA, WICBitmapDitherTypeNone, nil, 0.0, WICBitmapPaletteTypeMedianCut)) then
              begin
                // Setup Output Bitmap
                OutBitmap.PixelFormat := pf32bit;
                OutBitmap.SetSize(DestR.Right, DestR.Bottom);

                // Copy pixels directly to Bitmap memory
                Stride := OutBitmap.Width * 4;
                // WIC is Top-Down, TBitmap scanlines are accessible. Copy line by line to be safe.
                for I := 0 to OutBitmap.Height - 1 do
                begin
                   if Failed(Converter.CopyPixels(nil, Stride, Stride, OutBitmap.ScanLine[I])) then
                     Abort; // Break to fallback
                end;

                // Handle Orientation for WIC loaded images if needed (basic rotation)
                if ExifOrientation and ((Ext = '.jpg') or (Ext = '.jpeg')) then
                begin
                   // Re-read orientation via simple way or use existing logic if extracted above.
                   // For speed, we often rely on the fact that WIC *can* auto-rotate if configured,
                   // but here we just render. If you need rotation, apply SpPixelRotate here.
                end;
    ...
    // WIC failed (e.g. old Windows or weird format), fall through to standard loader
    Result := False;
  end;

  if Result then Exit;

  // 3. Fallback to standard VCL loading (Slow, mainly for WMF/EMF/ICO or if WIC failed)
  P := TPicture.Create;
  try
    if SpLoadGraphicFile(Filename, P, False) then
    begin
      ImageWidth := P.Graphic.Width;
      ImageHeight := P.Graphic.Height;
...
begin
  ...
  with FValidImageFormats do begin
    CommaText := '.jpg, .jpeg, .jpe, .jif, .bmp, .emf, .wmf, .png, .webp';
  end;
  ...
  if FillColors then begin
    for I := 0 to FValidImageFormats.Count - 1 do begin
      ...
      else if Pos(Ext, '.png, .webp') > 0 then FValidImageFormats.Colors[I] := $DAB6DA
