...
{$ENDIF LIBMPV_0_36_ABOVE}
    ...
    function SetViewWindow(hHandle: THandle): TMPVErrorCode;
  public
...
begin
  ...
  // If we are already loading the next file (via OpenFile),
  // ignore the "End" event of the previous file.
  if m_eState <> mpsLoading then
  begin
    // Check reason if needed:
    // pEF^.reason = MPV_END_FILE_REASON_EOF (Finished naturally)
    // pEF^.reason = MPV_END_FILE_REASON_STOP (Stopped by user/command)
    // pEF^.reason = MPV_END_FILE_REASON_ERROR (Error)

    if m_eState <> mpsEnd then
    begin
      // Only change state if we aren't already ended
      bChg := True;
    end;
  end;
...
//  s: string;
//begin
//  // this prevents property fetching errors if load failed #byvhl
//  if m_eState = mpsErr then Exit(MPV_ERROR_GENERIC);
//
//  m_fSpeed := 1.0;
//  m_fLenInSec := -1;
//  m_fLenMax := 0;
...
begin
  ...
  // CRITICAL FIX: Do NOT call GetProperty functions here.
  // They block the thread and cause the "Second Video Hang" issue.

  // 1. Reset values safely
  m_fSpeed := 1.0;
  // Note: m_fLenInSec and m_sFileName are now updated automatically
  // by the ID_DURATION and ID_PATH observers we added previously.

  // 2. Update Max length for EOF detection
  if m_fLenInSec > 0 then
    m_fLenMax := m_fLenInSec - SET_END_SUB_SECONDS
  else
    m_fLenMax := 0;

  // 3. Ensure video size is set (using cached observer values)
  DoSetVideoSize;
...
begin
  ...
  ID_PATH:
    begin
      // Capture the filename sinchrounously
      m_cLock.Enter;
      if pEP^.format = MPV_FORMAT_STRING then
        m_sFileName := string(UTF8String(PPMPVChar(pEP^.data)^));
      m_cLock.Leave;
    end;
  ID_DWIDTH:
    begin
      m_cLock.Enter;
      if pEP^.format = MPV_FORMAT_INT64 then
        m_nX := PInt64(pEP^.data)^;
      m_cLock.Leave;
      DoSetVideoSize;
    end;
  ID_DHEIGHT:
...
//      DoSetVideoSize;
//  end;
  // We shouldn't do query properties synchronously here, (commented out)
  // We now rely on ID_DWITH and ID_DHEIGHT events to update size (by vhl)
  // This prevents rendering starvation during window resize
  Result := MPV_ERROR_SUCCESS;
end;
...
// my events for resizing (by vhl)
  ObservePropertyInt64(STR_WIDTH, ID_WIDTH);
  ObservePropertyInt64(STR_HEIGHT, ID_HEIGHT);
  ObservePropertyString(STR_PATH, ID_PATH);
...
const
  ASYNC_REQ_ID = 1; // Any number > 0 triggers Async mode in our wrapper #byvhl
begin
  SetState(mpsLoading);
  //SetOptionString('idx', 'force');
  if bReset then
    Result := Command([CMD_LOAD_FILE, sFullName, 'replace'], ASYNC_REQ_ID)
  else
    Result := Command([CMD_LOAD_FILE, sFullName], ASYNC_REQ_ID);
  SetPropertyBool(STR_PAUSE, False, ASYNC_REQ_ID);
end;
...
function TMPVBasePlayer.SetViewWindow(hHandle: THandle): TMPVErrorCode;
begin
  Result := SetPropertyString(STR_WID, IntToStr(Int64(hHandle)));
end;
